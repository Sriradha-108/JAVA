...
| `38-Lambdas-and-Functional-Interface/` | `Demo.java` … `Demo6.java` | Functional interfaces in depth — `@FunctionalInterface` on a hand-written `Calculator`, the built-in `Function`, `Consumer`, `Supplier`, and `Predicate`, method references, and the composition methods `andThen`, `and`, `or`, and `negate` |
| `39-Introduction-to-Java-Stream/` | `Demo.java` | The stream pipeline — `stream()`, `filter()`, `map()`, `forEach()`, written first as separate assignments and then chained |
| `40-Java-Stream-Method/` | `Demo.java`, `Demo2.java`, `Demo3.java` | Stream methods by category — intermediate (`filter`, `map`, `flatMap`, `sorted`, `distinct`, `limit`, `skip`, `peek`), terminal (`reduce`, `collect`, `average`, the `match` family), and `Collectors` (`groupingBy`, `partitioningBy`, `joining`) |
| `41-Java-Optional/` | `Demo.java`, `Demo2.java` | `Optional` — `ofNullable`, `isPresent` / `ifPresent` / `ifPresentOrElse`, `orElse` / `orElseGet` / `orElseThrow`, and `map` / `flatMap` / `filter` replacing a nested null-check ladder |
| `42-Java-Parallel-Stream/` | `Demo.java` | `parallelStream()` next to `stream()`, and `forEachOrdered()` as the way to keep encounter order when the work is split across threads |
| `43-Java-Exceptional-Handling/` | `Demo.java` … `Demo4.java` | Why exception handling exists — an uncaught `ArithmeticException` ending the program, the same one propagating up a chain of methods, `try`/`finally`, and finally a `catch` that lets execution continue |
| `44-Exception-Hierarchy/` | `Demo.java` … `Demo5.java` | The exception hierarchy in use — nested `try` blocks handled inside and outside, catch ordering from specific to general with a multi-catch, a custom checked `InvalidAgeException` with `throw` and `throws`, and a checked `FileNotFoundException` |
| `45-Java-Memory-Management/` | `Demo.java` | Heap exhaustion — a loop allocating 1 MB arrays and holding on to every one, until the JVM throws `OutOfMemoryError` |
| `46-Java-Heap-Memory/` | `Demo.java` | Placeholder for the heap-memory examples |
| `47-Multithreading-in-java/` | `Demo.java` | Placeholder for the multithreading introduction |
| `48-Java-ThreadCreation-Lifecycle/` | `Demo.java` … `Demo6.java` | Creating threads and their lifecycle — `extends Thread` versus `implements Runnable`, `getName()` / `getId()`, why `start()` cannot be called twice, non-deterministic interleaving of two threads, and `getState()` across `NEW`, `RUNNABLE`, and `TERMINATED` |
| `49-Thread-Methods/` | `Demo.java` … `Demo8.java` | The `Thread` methods — `sleep()`, `join()`, `yield()`, `interrupt()`, `isAlive()`, `setName()` / `currentThread()`, `setPriority()`, and `setDaemon()` |
| `50-Problem-in-multithreading/` | `Demo.java`, `Demo2.java` | What goes wrong without coordination — two threads incrementing a shared counter, fixed with a `synchronized` block, and a `volatile` flag one thread writes and another reads |
| `51-Locks-in-Java/` | `Demo.java` … `Demo6.java` | Intrinsic locks — a `synchronized` method serialising two threads, two `synchronized` methods sharing one object lock, a `synchronized` block on the wrong object, static synchronization on `Counter.class`, and the class lock and instance lock being independent |
| `52-Inter-Thread-Communication/` | `Demo.java`, `Demo2.java`, `Demo3.java` | Producer-consumer in three steps — no coordination at all, then `synchronized` with a busy-wait that deadlocks, then the working version with `wait()` and `notify()` |
| `53-Java-Locks/` | `Demo.java`, `Demo2.java`, `Demo3.java` | The explicit locks in `java.util.concurrent.locks` — `ReentrantLock` with `unlock()` in a `finally`, `ReentrantReadWriteLock` sharing reads and excluding writes, and `StampedLock` with an optimistic read that validates and falls back |
...
- `@FunctionalInterface` is optional but worth writing. It changes nothing at run time; it makes the compiler reject the interface if a second abstract method is ever added, which is exactly when the lambdas using it would break.
- The four built-in shapes cover most cases: `Function<T,R>` takes one and returns one (`apply`), `Consumer<T>` takes one and returns nothing (`accept`), `Supplier<T>` takes nothing and returns one (`get`), and `Predicate<T>` takes one and returns a `boolean` (`test`).
- `System.out::println` is a method reference — the same lambda with the parameter left implicit. Any lambda that does nothing but pass its argument straight to one method can be written this way.
- `Function.andThen(g)` runs the receiver first and `compose(g)` runs the argument first, so `add2.andThen(multiply3)` is `(x + 2) * 3` while `add2.compose(multiply3)` would be `x * 3 + 2`. Composition returns a new `Function`; nothing runs until `apply` is called.
- `Predicate` composes the same way with `and`, `or`, and `negate`, standing in for `&&`, `||`, and `!`. `38-.../Demo5.java` builds `passed.or(isAdult)` once and tests students against it.
- `Consumer.andThen` chains side effects instead of values — both consumers receive the same input, in order, which is why `Demo6.java` prints the name and then its uppercase form.
- A stream is a pipeline, not a collection. It holds no data, it does not change the source, and it can be used only once — a second terminal operation on the same stream throws `IllegalStateException`.
- Intermediate operations (`filter`, `map`, `sorted`, `distinct`, `limit`, `skip`, `peek`) are lazy and return a new stream. Nothing runs until a terminal operation (`forEach`, `collect`, `reduce`, `count`, `average`, the `match` family) asks for a result.
- `sorted()` and `distinct()` are stateful — they have to see the whole stream before they can emit anything, unlike `filter` and `map`, which handle one element at a time. `limit` and `findFirst` are the opposite: they short-circuit and stop early.
- `flatMap` flattens one level, turning a stream of lists into a stream of their elements. `map` would leave a `Stream<List<Integer>>` instead.
- `peek` is for debugging, not for output. It exists to look at elements mid-pipeline, and like every intermediate operation it does nothing without a terminal one.
- `average()`, `sum()`, `max()`, and `min()` live on the primitive streams, which is why `40-.../Demo2.java` calls `mapToInt` first. `average()` returns an `OptionalDouble`, because an empty stream has no average.
- `Collectors` is where a stream turns back into a collection: `toList`, `toSet`, `toMap`, `joining` for strings, `groupingBy` for a `Map` of buckets, and `partitioningBy` for the two-bucket `true`/`false` case. `groupingBy` takes a second collector — `Collectors.mapping(...)` — to transform each group as it is built.
- `Optional` is a container that either holds a value or is empty, and its point is to make "might be absent" part of the type. `Optional.of` rejects `null`, `ofNullable` accepts it, and `empty()` is the empty one.
- `orElse` always evaluates its argument, `orElseGet` takes a supplier and evaluates it only when the value is missing, and `orElseThrow` throws instead. Use `orElseGet` when the fallback is expensive to build.
- `map`, `flatMap`, and `filter` work on an `Optional` the way they work on a stream, which is what collapses the nested null checks in `41-.../Demo2.java` into one chain. `flatMap` is the one to use when the field is itself an `Optional`.
- `Optional.get()` throws on an empty value, so it is the method to avoid — `ifPresent`, `ifPresentOrElse`, and the `orElse` family cover the same ground safely.
- `parallelStream()` splits the work across the common ForkJoin pool. It is worth it only for large data and genuinely independent, stateless operations; for small lists the coordination costs more than it saves.
- `forEach` on a parallel stream gives no order guarantee — `forEachOrdered` restores it at the cost of some of the parallelism. Anything that mutates shared state inside a parallel stream is a race, which is why the operations have to be stateless.
- An uncaught exception ends the program: `43-.../Demo.java` prints `Step 1`, throws on `a / b`, and never reaches `Step 2`. The exception also travels back up the call stack, so `Demo2.java` fails in `methodB` and takes `methodA` and `main` down with it.
- `finally` always runs, but it does not handle anything. `Demo3.java` has `try`/`finally` with no `catch`, so `"Heyyyy"` prints and then the `ArithmeticException` continues up and kills the program — `Step 2` still never runs. Only a `catch` lets execution carry on, which is what `Demo4.java` adds.
- An exception can be caught anywhere up the chain, not only where it was thrown. `44-.../Demo.java` handles it in the inner `catch`; in `Demo2.java` the inner `catch` is for the wrong type, so the outer one takes it instead.
- `catch` blocks are checked in order, so they must run from most specific to most general — a `catch (Exception e)` first would shadow everything after it. Multi-catch, `catch (A | B e)`, is for two unrelated types that need identical handling.
- `throw` raises an exception, `throws` declares that a method may raise one. Checked exceptions — anything extending `Exception` but not `RuntimeException` — must be either caught or declared, which is why `checkEligibility` says `throws InvalidAgeException` and `readFile` says `throws FileNotFoundException`.
- A custom exception is a class extending `Exception` (checked) or `RuntimeException` (unchecked). Passing the message to `super(message)` is what makes `getMessage()` work, and extra fields like `age` in `InvalidAgeException` can carry context the message cannot.
- `45-Java-Memory-Management/Demo.java` allocates 1 MB arrays in an infinite loop and keeps a reference to every one, so nothing is ever garbage collected and the JVM ends in `OutOfMemoryError: Java heap space`. It is meant to fail — run it with `-Xmx64m` to reach the error quickly instead of grinding the machine.
- `44-Exception-Hierarchy/Demo6.java`, `Demo7.java`, `45-Java-Memory-Management/Demo2.java`, and `46-Java-Heap-Memory/Demo.java` are still empty skeletons, kept as placeholders for the next examples.
- There are two ways to make a thread: `extends Thread` or `implements Runnable`. `Runnable` is the better default, because a class can implement it and still extend something else, and because it separates the task from the worker — which is also why it is a functional interface and can be written as a lambda.
- `start()` and `run()` are not the same call. `start()` asks the JVM for a new OS thread with its own stack and program counter, and that thread calls `run()`; calling `run()` directly is an ordinary method call on the current thread, with no concurrency at all.
- A `Thread` object cannot be restarted. Once it has been started it never returns to `NEW`, so the second `t1.start()` in `48-.../Demo4.java` throws `IllegalThreadStateException` — the demonstration that file is making.
- Interleaving is not deterministic. The two loops in `Demo5.java` produce a different mix on every run and on every machine, which is why correctness must never depend on the order two threads happen to print in.
- The lifecycle states are `NEW`, `RUNNABLE`, `BLOCKED`, `WAITING`, `TIMED_WAITING`, and `TERMINATED`, and `getState()` reports them — `Demo6.java` samples the same thread before `start()`, just after it, and once it has finished. `RUNNABLE` covers both running and waiting for a CPU; Java does not separate the two.
- `Thread.sleep(ms)` moves the current thread to `TIMED_WAITING` and does not release any lock it holds. It throws `InterruptedException`, which is checked, so every call sits in a `try` or a method that declares it.
- `join()` makes the caller wait for the other thread to finish. `t1.join(1000)` waits at most a second and then continues regardless, which is why `49-.../Demo2.java` can print its last line before `t1` does.
- `Thread.yield()` is only a hint that the current thread is willing to give up its slot. The scheduler may ignore it entirely, and the thread stays `RUNNABLE` rather than moving to a waiting state.
- `interrupt()` does not stop a thread. It sets an interrupt flag the thread is expected to check — which the loop in `Demo4.java` does — and a thread parked in `sleep()`, `join()`, or `wait()` gets an `InterruptedException` instead. The flag starts out `false`, and `isInterrupted()` reads it while the static `Thread.interrupted()` reads *and clears* it.
- `isAlive()` is true between `start()` and termination, and false on either side of that window.
- Thread priority is a hint too. `MIN_PRIORITY` is 1, `NORM_PRIORITY` 5, and `MAX_PRIORITY` 10, but the OS decides how much notice to take — and setting it after `start()`, as `Demo7.java` does, is later still than the scheduler needs it.
- A daemon thread does not keep the JVM alive: once the last non-daemon thread ends, the JVM exits and kills every daemon mid-work. `setDaemon(true)` has to be called before `start()`. The garbage collector runs on daemon threads, which is the reason they exist.
- `count++` is three operations — read, add, write — so two threads can read the same value and one increment is lost. Without coordination the counter in `50-.../Demo.java` ends below 20000, and by a different amount each run.
- `synchronized` fixes that by letting one thread at a time hold the object's monitor. The block form, `synchronized (this) { ... }`, locks only the critical section rather than the whole method, which is what keeps the surrounding work parallel.
- `volatile` is about visibility, not atomicity. Without it, a thread may keep reading its own cached copy of `flag` and spin forever; with it, every read goes to main memory. It fixes the loop in `Demo2.java`, but it would not fix `count++`, which needs `synchronized` or an `AtomicInteger`.
- `47-Multithreading-in-java/Demo.java` is still an empty skeleton, kept as a placeholder for the introduction.
- Every object owns one intrinsic lock, or monitor, and `synchronized` acquires it. A `synchronized` instance method locks `this`, so the two threads calling `test.show()` in `51-.../Demo.java` take it in turn rather than overlapping.
- That lock is per object, not per method, so *all* the `synchronized` methods of one object share it. `m1` and `m2` in `Demo2.java` never run at the same time on the same `Test`, even though they touch nothing in common — which is the cost of locking the whole object instead of a block.
- The block form locks whatever object is named, so separate locks give separate critical sections — the `lock1` and `lock2` fields in `Demo3.java` let `deposit()` and `withdraw()` proceed independently.
- The lock object has to be shared to do anything. `synchronized (new Object())` in `Demo3.java`'s `m1()` takes a fresh lock on every call, so no two threads ever contend and the block excludes nothing — a lock that looks right and does nothing.
- A `static synchronized` method locks the `Class` object instead of an instance, which is what `synchronized (Counter.class)` in `Demo4.java` writes out explicitly. It is the only way to guard `static` state, since there is no `this`.
- The class lock and the instance lock are two different monitors, so a static method and an instance method never block each other — `Demo5.java` shows `m1` and `m2` overlapping for exactly that reason.
- Without any coordination, the producer and consumer in `52-.../Demo.java` simply miss each other: items are overwritten before being read and read twice after being consumed, because nothing makes either thread wait for the other.
- `52-.../Demo2.java` adds `synchronized` and a `while` loop that spins on the flag, and that combination **deadlocks**. The waiting thread holds the object's monitor while it spins, so the other thread can never enter the method that would change the flag. Busy-waiting inside a lock is the mistake the next file fixes.
- `wait()` releases the monitor and parks the thread; `notify()` wakes one waiter and `notifyAll()` wakes all of them. Releasing the lock is the whole difference from a spin loop, and it is why `Demo3.java` works where `Demo2.java` hangs.
- All three must be called while holding the monitor of the object they are called on, or they throw `IllegalMonitorStateException`. Always wait in a `while` loop rather than an `if` — a thread can wake without the condition being true.
- `ReentrantLock` is the explicit version of `synchronized`: `lock()` and `unlock()` instead of a block. `unlock()` belongs in a `finally`, as in `53-.../Demo.java`, because nothing releases it automatically when an exception unwinds the method.
- What it buys over `synchronized` is control — `tryLock()` to give up rather than block, `lockInterruptibly()` to stay cancellable, a fairness option, and the freedom to lock and unlock in different scopes. "Reentrant" means the thread already holding it can take it again, which `synchronized` also does.
- `ReentrantReadWriteLock` splits the two: any number of readers may hold the read lock at once, while the write lock is exclusive. It pays off when reads greatly outnumber writes, which is the shape `Demo2.java` sets up with three readers and three writers.
- `StampedLock` adds an optimistic read that takes no lock at all — `tryOptimisticRead()` returns a stamp, the value is read, and `validate(stamp)` reports whether a writer intervened. Only when validation fails does `Demo3.java` fall back to a real read lock. It is the fastest of the three for read-heavy work, but it is not reentrant.
- The files inside `51-Locks-in-Java/`, `52-Inter-Thread-Communication/`, and `53-Java-Locks/` each redeclare the same helper class — `Test`, `Box`, and `SharedResource` — so compile them one at a time from their own folder.
- `51-Locks-in-Java/Demo6.java` is still an empty skeleton, kept as a placeholder.
